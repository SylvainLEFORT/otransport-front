...
const PendingDeliveries = () => {
  ...
  useEffect(() => {
    axios.get('http://celia-tosic-server.eddi.cloud/projet-03-o-transport-back/public/api/admin/deliveries/pending', config)
      .then((res) => {
  ...
  const deleteDelivery = (readDeliveryId) => async () => {
    ...
    if (confirmed) {
      await axios.delete(`http://celia-tosic-server.eddi.cloud/projet-03-o-transport-back/public/api/admin/deliveries/${readDeliveryId}`, config);
      setStatus(true);
